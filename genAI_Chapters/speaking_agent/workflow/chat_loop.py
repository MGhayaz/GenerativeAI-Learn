...
try :
    while True:
        user_query = speech.stt.speech_to_text(audio= speech.microphone.Recognizer() )
        print(f"User: ", user_query)
        if user_query.lower().strip() in ["exit", "band hojao","bye"]:
            print("Irshard V2: Acha waqt bacha raha toh phir milinge")
            break
        history_log = history.append_user_query( user_audio_to_text= user_query )
        ...
        interaction = speech.tts.generate_tts(final_content=final_content)
        print("🗣️LLM:", final_content)
        
        print("playing ai audio")
        wav_file = speech.audio.wave_file('out.wav', base64.b64decode(interaction.output_audio.data))
        try:
            speech.audio.play_audio("out.wav")
